...
struct ssi_internal_ctx {
    ...
    char         error_no_msg; /* use error 500x instead include error message */
                                   ...
                                   apr_bucket_brigade *bb)
{
    ...
    if (last_error) {
        SSI_CREATE_ERROR_BUCKET(ctx, f, bb);
        if (ctx->intern->error_no_msg > 0) return APR_ENOSTAT;
    }
...
static apr_status_t includes_filter(ap_filter_t *f, apr_bucket_brigade *b)
{
    ...
    if (!f->ctx) {
        ...
        intern->error_no_msg = (conf->error_no_msg > 0);
...
static void *merge_includes_dir_config(apr_pool_t *p, void *basev, void *overridesv)
{
    ...
    MERGE(base, over, new, error_no_msg,      UNSET);
    return new;
}
...
static const command_rec includes_cmds[] =
{
    ...
    AP_INIT_FLAG("SSIErrorParser", ap_set_flag_slot_char,
                  (void *)APR_OFFSETOF(include_dir_config, error_no_msg),
                  OR_LIMIT,
                  "Whether to use Apache error 500x or include error message. "
